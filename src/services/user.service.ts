...
export class UserService implements OnModuleInit {
    ...
    public async register(createUserData: any): Promise<UserEntity> {
     ...
     * @param password 登录密码
     */
    public async login(account: string, password: string): Promise<UserEntity> {
     ...
     */
    public async update(id: number, updateInput: UserEntity) {
        const existing = await this.userRepository.findOne(id);
        if (!existing) {
     ...
     * @param account 登录账号
     */
    public async findOneByAccount(account: string): Promise<UserEntity> {
     ...
     * @param id 用户ID
     */
    public async findOneWithPostsById(id: number): Promise<UserEntity> {
